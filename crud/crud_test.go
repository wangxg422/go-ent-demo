...
func TestEntRelationsWithAssert(t *testing.T) {
    ...
    // --- 2. 创建角色 ---
    roleAdmin, err := client.SysRole.Create().SetName("管理员").Save(ctx)
    assert.NoError(err)
    roleUser, err := client.SysRole.Create().SetName("普通用户").Save(ctx)
    assert.NoError(err)
    ...
    // --- 6. 测试查询角色的用户 ---
    usersWithAdmin, err := client.SysRole.Query().Where(sysrole.IDEQ(roleAdmin.ID)).QueryUsers().All(ctx)
    assert.NoError(err)
    userNames := []string{}
